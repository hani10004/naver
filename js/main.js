...
let btn =document.querySelector('#phoneNo');
...
function checkId(){
    ...
    console.log(idPattern.test(id.value)) //true;false
    if(id.value === ""){
        error[0].innerHTML="필수 정보입니다."
        error[0].style.display="block"
        error[0].style.color="#f00"
    }else if(!idPattern.test(id.value)){
        /* !--> 부정을 나타냄/ 사용자가 입력한 입력값이 틀리면(정규식에 일치하지않으면) true로 표시 */
        error[0].innerHTML="5~20자의 영문 소문자, 대문자, 숫자와 특수기호(_),(-)만 사용 가능합니다."
        error[0].style.display="block"
        error[0].style.color="#f00"
    }else{
        error[0].innerHTML="멋진 아이디에요!"
...
function isBirthCompleted(){
    ...
    if(!yearPattern.test(yy.value)){
        ...
        //년도가(정규식 규칙에 맞게 입력했다면) 맞다면 월을 체크한다
        isMonthCompleted()
    }
    ...
    function isDateCompleted(){
        if(dd.value == ""){
            error[4].innerHTML= "태어난 일 2자리를 정확하게 입력하세요."
            error[4].style.display="block"
        }else{
            //
            isBirthRight();
